...
# Noise removal with iterative bilateral filter(removes noise while
# preserving edges)
...
# Morphological opening with a rectangular structure element
    ...
    morph_image = cv2.morphologyEx(
        equal_histogram, cv2.MORPH_OPEN, kernel, iterations=300)

# Image subtraction(Subtracting the Morphed image from the histogram
# equalised Image)
...
# Finding Contours in the image based on edges
    new, contours, hierarchy = cv2.findContours(
        thresh_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    contours = sorted(contours, key=cv2.contourArea, reverse=True)[:10]
# Sort the contours based on area ,so that the number plate will be in top
# 10 contours
    screenCnt = None
...
# Applying histogram equalisation
    ...
    morph_image1 = cv2.morphologyEx(
        equal_histogram1, cv2.MORPH_OPEN, kernel1, iterations=150)
    sub_morp_image1 = cv2.subtract(equal_histogram1, morph_image1)
    ret1, thresh_image1 = cv2.threshold(
        sub_morp_image1, 0, 255, cv2.THRESH_OTSU)
    path = "C:\\Users\\Vamsi\\Desktop\\ser\\sample"
    cv2.imwrite(os.path.join(path,'now' + str(count) + '.JPG'), thresh_image1)
    count = count + 1
